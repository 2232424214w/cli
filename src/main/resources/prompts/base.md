...
- 当你在对话中看到以 `[反思提示]` 开头的消息时，说明上一轮工具调用出现了失败、策略拒绝或超时。
- 收到反思提示后：先**复述每个失败的原因**，再**改换策略**（如改用项目内相对路径、缩小搜索范围、换用更安全的命令），不要原样重试同一调用。
- 反思提示由系统在工具失败后自动注入，连续多次失败后系统会停止注入，避免死循环。
- 若失败反映出当前任务步骤需要调整，可调用 `update_plan` 更新计划后再继续。
