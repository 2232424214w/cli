package com.bettercli.snapshot;
...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("bettercli.snapshot.enabled", "BETTERCLI_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("bettercli.snapshot.dir", "BETTERCLI_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".bettercli", "snapshots").toString()));
        int max = readInt("bettercli.snapshot.max", "BETTERCLI_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("bettercli.snapshot.excludes", "BETTERCLI_SNAPSHOT_EXCLUDES", ""));
