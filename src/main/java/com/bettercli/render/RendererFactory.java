...
import com.bettercli.render.inline.TerminalCapabilities;
 ...
 * <p>选型规则：
 * <ul>
 *   <li>{@code -Dbettercli.renderer} > {@code BETTERCLI_RENDERER} 环境变量 > 默认 inline</li>
 ...
 *   <li>兼容：{@code BETTERCLI_TUI=true} → 等价 {@code lanterna}，打 deprecation 提示</li>
 * </ul>
 *
...
public final class RendererFactory {
    ...
    public static Mode resolveMode() {
        ...
        // 兼容旧 BETTERCLI_TUI=true → lanterna
        String legacyTui = System.getenv("BETTERCLI_TUI");
        if (legacyTui != null && Boolean.parseBoolean(legacyTui.trim())) {
            System.err.println("⚠️ BETTERCLI_TUI=true 已废弃，请改用 BETTERCLI_RENDERER=lanterna");
    ...
    private static Mode parse(String raw) {
        return switch (raw.trim().toLowerCase()) {
            ...
            default -> {
                System.err.println("⚠️ 未识别的 BETTERCLI_RENDERER='" + raw + "'，回退到 inline");
                yield Mode.INLINE;
            }
        };
    }
