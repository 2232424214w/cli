...
public class RuntimeApiServer implements AutoCloseable {
    ...
    private final ExecutorService executor = Executors.newCachedThreadPool(r -> {
        Thread thread = new Thread(r, "bettercli-runtime-api");
    ...
    public RuntimeApiServer(RuntimeThreadStore store, TaskRunner runner, int port, String apiKey) throws IOException {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalArgumentException("Runtime API 需要配置 BETTERCLI_RUNTIME_API_KEY 或 -Dbettercli.runtime.api.key");
        }
        this.store = store;
        this.runner = runner;
        this.apiKey = apiKey;
    ...
    public static String configuredApiKey() {
        String configured = System.getProperty("bettercli.runtime.api.key");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("BETTERCLI_RUNTIME_API_KEY");
        }
        return configured;
    }
    ...
    private boolean authorized(HttpExchange exchange) {
        ...
        String direct = exchange.getRequestHeaders().getFirst("X-BetterCLI-API-Key");
