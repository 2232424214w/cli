 ...
 * 1. 系统属性：{@code bettercli.react.token.budget} / {@code bettercli.react.stagnation.window} /
 *    {@code bettercli.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dbettercli.react.token.budget=N} 显式启用。
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dbettercli.react.token.budget=N 仍可启用硬预算，覆盖默认。
        return new AgentBudget(
                readIntProperty("bettercli.react.token.budget", Integer.MAX_VALUE),
                readIntProperty("bettercli.react.stagnation.window", DEFAULT_STAGNATION_WINDOW),
                readIntProperty("bettercli.react.hard.max.iterations", DEFAULT_HARD_MAX_ITERATIONS)
        );
    }
