...
public final class WechatPaths {
    ...
    public static Path root() {
        String configured = System.getProperty("bettercli.wechat.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("BETTERCLI_WECHAT_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".bettercli", "wechat").toString();
        }
    ...
    public static Path pidFile() {
        return root().resolve("bettercli-wechat.pid");
    }
}
