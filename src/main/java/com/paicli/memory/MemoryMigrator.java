 ...
 * <p>交付时用法：
 * <pre>{@code
 * paicli memory migrate --from sqlite --to postgres \
 *     --postgres-url=jdbc:postgresql://host:5432/paicli \
 *     --postgres-user=paicli --postgres-password=***
 * }</pre>
 *
 * <p>迁移范围：
 * <ul>
    ...
    public int migrate() {
        ...
        throw new UnsupportedOperationException(
                "MemoryMigrator 尚未交付：需要 PostgreSQL 后端就绪后实现。"
                        + "当前请继续使用 SQLite（PAICLI_MEMORY_BACKEND=sqlite）");
    }
}
